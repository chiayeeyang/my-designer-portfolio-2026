...
export default function PolyShowSlideshow({ images }) {
    ...
    return <div ref={ref} className={styles.slideshow} role="region" aria-roledescription="carousel" aria-label="Graduate show photographs"><div className={styles.stage}>{images.map((image, i) => <img key={image.src} src={image.src} alt={image.label} hidden={i !== active}/>)}</div><div className={styles.controls}><div><span>{String(active + 1).padStart(2, '0')} / {String(images.length).padStart(2, '0')}</span><p>{images[active]?.label}</p></div><div className={styles.buttons}><button type="button" aria-label="Previous photograph" onClick={() => move(-1)}>←</button>{!reduced && <button type="button" onClick={() => setPaused(!paused)} aria-label={paused ? 'Play slideshow' : 'Pause slideshow'}>{paused ? 'Play' : 'Pause'}</button>}<button type="button" aria-label="Next photograph" onClick={() => move(1)}>→</button></div></div></div>;
}
