...
const attachmentVariants = cva('rounded-xl w-fit focus-within:ring-1 focus-within:ring-ring/50 group/attachment relative flex max-w-full min-w-0 shrink-0 flex-wrap border bg-card text-card-foreground transition-colors has-[>a,>button]:hover:bg-muted/50 data-[state=error]:border-destructive/30 data-[state=idle]:border-dashed', {
    variants: {
        size: {
...
function AttachmentContent({ className, ...props }) {
    return (<div data-slot="attachment-content" className={cn('leading-tight group-data-[orientation=vertical]/attachment:px-1 max-w-full min-w-0 flex-1', className)} {...props}/>);
}
function AttachmentTitle({ className, ...props }) {
    return (<span data-slot="attachment-title" className={cn('font-medium block max-w-full min-w-0 truncate group-data-[state=processing]/attachment:shimmer group-data-[state=uploading]/attachment:shimmer', className)} {...props}/>);
}
function AttachmentDescription({ className, ...props }) {
    return (<span data-slot="attachment-description" className={cn('mt-0.5 text-xs block min-w-0 truncate text-muted-foreground group-data-[state=error]/attachment:text-destructive/80', 'max-w-full', className)} {...props}/>);
}
function AttachmentActions({ className, ...props }) {
    return (<div data-slot="attachment-actions" className={cn('group-data-[orientation=vertical]/attachment:absolute group-data-[orientation=vertical]/attachment:top-3 group-data-[orientation=vertical]/attachment:right-3 relative z-20 group-data-[orientation=vertical]/attachment:gap-1 flex shrink-0 items-center', className)} {...props}/>);
}
...
function AttachmentGroup({ className, ...props }) {
    return (<div data-slot="attachment-group" className={cn('gap-3 scroll-px-1 py-1 flex min-w-0 scroll-fade-x snap-x snap-mandatory scrollbar-none overflow-x-auto overscroll-x-contain *:data-[slot=attachment]:flex-none *:data-[slot=attachment]:snap-start', className)} {...props}/>);
}
