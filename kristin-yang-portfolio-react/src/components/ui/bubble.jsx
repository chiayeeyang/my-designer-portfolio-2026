...
const bubbleVariants = cva('gap-1 data-[align=end]:self-end max-w-[80%] data-[variant=ghost]:max-w-full group-data-[align=end]/message:self-end group/bubble relative flex w-fit min-w-0 flex-col', {
    variants: {
        variant: {
...
function BubbleContent({ className, render, ...props }) {
    return useRender({
        defaultTagName: 'div',
        props: mergeProps({
            className: cn('rounded-xl border border-transparent px-3 py-2 text-sm leading-relaxed [button,a]:outline-none [button,a]:focus-visible:border-ring [button,a]:focus-visible:ring-3 [button,a]:focus-visible:ring-ring/50 group-data-[align=end]/bubble:self-end w-fit max-w-full min-w-0 overflow-hidden wrap-break-word [button]:text-left [button,a]:transition-colors', className),
        }, props),
        render,
        state: {
            slot: 'bubble-content',
...
const bubbleReactionsVariants = cva('rounded-full ring-3 ring-card bg-muted shrink-0 gap-1 px-1.5 py-0.5 has-[button]:p-0 text-sm absolute z-10 flex w-fit items-center justify-center', {
    variants: {
        side: {
